...
RSpec.describe LlmCoverLetterController, type: :request do
  ...
  let(:valid_attributes) { { job_application_id: job_application.id, temperature: '0.5' } }
  ...
  describe 'POST /llm_cover_letter/' do
    context 'with valid parameters' do
      it 'enqueues a job for the llm to create the cover letter' do
        ...
        end.to have_enqueued_job(CreateLlmCoverLetterJob).with(job_application.id, '0.5')
      end
    ...
    context 'with invalid parameters' do
      it 'redirects to the root path' do
        post '/llm_cover_letter', params: { job_application_id: 0, temperature: '0.5' }
